...
class ProductListBloc extends Bloc<ProductListEvent, ProductListState>{
  ProductListBloc():super(ProductListInitial()) {
    on<ProductListAttempt>(_getProductList);
   }

    Future<void>_getProductList(ProductListAttempt event, Emitter<ProductListState> emit)async{
      emit(ProductListInitial());
      try{
          final response = await ApiService.getProductListApi(event.isOnline);


          if(response["status"] == true){
            final List<Map<String,dynamic>> data = List<Map<String,dynamic>>.from(response['message']['products']);
            print("Bloc service result $data");
            final prefs = await SharedPreferences.getInstance();
            final jsonData = jsonEncode(data);
            prefs.setString("data", jsonData);
            emit(ProductListSuccess(data));
          } else {
            emit(ProductListFailure(response['message']));
          }
      } catch(e){
